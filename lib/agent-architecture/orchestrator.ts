import { EmailContext, EnrichmentField, RowEnrichmentResult } from './core/types';
import { EnrichmentResult, SearchResult } from '../types';
...
export class AgentOrchestrator {
  ...
  private async runDiscoveryPhase(
    ...
    if (ctxEmailContext.companyDomain) {
      ...
      if (onAgentProgress) {
        onAgentProgress(`Accessing ${ctxEmailContext.companyDomain}...`, 'info');
      }
      try {
        ...
        if (scraped.data && scraped.data.markdown && this.isValidCompanyWebsite({ markdown: scraped.data.markdown, metadata: scraped.data as Record<string, unknown> })) {
          ...
          if (onAgentProgress) {
            onAgentProgress(`Successfully accessed company website`, 'success');
          }
        ...
        if (onAgentProgress) {
          onAgentProgress(`Direct website access failed, initiating search fallback`, 'warning');
        }
      }
    } else {
    ...
    if (missingFields.length > 0) {
      ...
      for (const query of searchQueries) {
        ...
        try {
          ...
          if (onAgentProgress && searchQueries.indexOf(query) === 0) {
            onAgentProgress(`Searching for company information...`, 'info');
          }
  ...
  private async runProfilePhase(
    ...
    if (onAgentProgress) {
      onAgentProgress(`Searching for ${String(companyName)} profile information...`, 'info');
    }
    ...
    if (onAgentProgress && searchResults.length > 0) {
      onAgentProgress(`Found ${searchResults.length} sources with profile data`, 'info');
    }
  ...
  private async runGeneralPhase(
    ...
    if (companyDomain && this.hasExecutiveFields(fields)) {
      ...
      for (const url of executiveUrls) {
        try {
